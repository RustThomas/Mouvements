...
def calculate_regression_metrics(y_true, y_pred, y_true_val=None, y_pred_val=None):
    ...
    print("squeeze")
    y_ecart=abs(np.squeeze(y_pred)-y_true)
    print("a",y_ecart)
    print("y_true", y_true)

    Aad=0
    Moy=np.average(y_ecart)
    for e in range(len(y_ecart)):
      Aad+=abs((y_ecart[e]-Moy))
    #print("Add:",Aad/len(y_ecart))

    print('AAD :',Aad/len(y_ecart))
